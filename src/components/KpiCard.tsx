    ...
    <div key={card.name} className='bg-surface dark:border-1 border-primary rounded-xl shadow p-6 flex flex-col gap-4 justify-center col-span-12 xl:col-span-4 lg:col-span-6'>
      <div className='flex justify-between gap-5 items-center'>
        ...
        <div className="flex gap-1 items-baseline">
          <div
            className={`cursor-default rounded-xl flex justify-center items-center gap-2 px-4 text-sm md:text-base font-semibold ${card.variancePercentage > 0 ? 'bg-green-100 text-green-900 dark:bg-transparent dark:text-emerald-500' : 'bg-rose-100 text-red-900 dark:bg-transparent dark:text-rose-600'}`}>
      ...
      <div className="flex gap-2">
        <h2 className='text-2xl font-semibold dark:text-white'>{formatNumber(card.amount, 2)}$</h2>
      </div>
